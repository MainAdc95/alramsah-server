...
function homeInfo(req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var sections, _i, sections_1, section, news, strips, tmrNews, article, err_10;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 9, , 10]);
...
function getStatistics(req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var dataType, d, date, sections, news, visitors, newsPerDay, latestNews, err_11;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 6, , 7]);
                ...
                case 2:
                    ...
                    return [4 /*yield*/, db_1.pool.query("\n                SELECT\n                    visitor_id,\n                    user_data,\n                    created_at\n                FROM visitors\n                WHERE created_at > $1\n                ORDER BY created_at desc\n            ", [new Date(date)])];
                case 3:
                    visitors = (_a.sent()).rows;
                    return [4 /*yield*/, db_1.pool.query("\n            SELECT\n                created_at\n            FROM news\n            WHERE created_at > $1\n            ORDER BY created_at desc\n            ", [new Date(date)])];
                case 4:
                    newsPerDay = (_a.sent()).rows;
                    return [4 /*yield*/, db_1.pool.query("\n            SELECT\n                n.news_id,\n                n.title,\n                n.readers,\n                is_archived,\n                n.created_at,\n                CASE WHEN COUNT(i)=0 THEN ARRAY[]::jsonb[] ELSE array_agg(DISTINCT i.image) END as images,\n                CASE WHEN COUNT(s)=0 THEN null ELSE jsonb_build_object (\n                    'section_id', s.section_id,\n                    'section_name', s.section_name,\n                    'color', s.color\n                ) END as section,\n                CASE WHEN COUNT(tn)=0 THEN null ELSE jsonb_build_object (\n                    'image_id', tn.image_id,\n                    'sizes', tn.sizes\n                ) END as thumbnail\n            FROM news n\n                LEFT JOIN users cb ON cb.user_id=n.created_by\n                LEFT JOIN users ub ON ub.user_id=n.updated_by\n                LEFT JOIN sections s ON s.section_id=n.section\n                LEFT JOIN images tn ON tn.image_id=n.thumbnail\n                LEFT JOIN (\n                    SELECT\n                        ni.news_id,\n                        jsonb_build_object (\n                            'image_id', i.image_id,\n                            'sizes', i.sizes\n                        ) as image\n                    FROM news_image ni\n                        LEFT JOIN images i ON i.image_id=ni.image_id\n                ) as i\n                    ON i.news_id=n.news_id\n            WHERE n.created_at > $1\n            GROUP BY n.news_id, s.section_id, tn.image_id\n            ORDER BY n.readers desc\n            LIMIT 20\n            ", [new Date(date)])];
                case 5:
                    latestNews = (_a.sent()).rows;
                    return [2 /*return*/, res
                            .status(200)
                            .json({ sections: sections, news: news, newsPerDay: newsPerDay, latestNews: latestNews, visitors: visitors })];
                case 6:
                    err_11 = _a.sent();
